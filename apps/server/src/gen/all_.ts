...
export namespace ping2 {
    ...
    export class Input {
        ...
        static fromTyped({        
            ...
            return new ping2.Input({"fn.ping2": input});
        }
    }
    ...
    export class Output {
        ...
        static from_Ok_(payload: ping2.Output.Ok_): ping2.Output {
            return new ping2.Output({
                "Ok_": payload.pseudoJson
            });
        }

        getTaggedValue():
            TaggedValue_<"Ok_", ping2.Output.Ok_> | UntypedTaggedValue_ {
            const tag = Object.keys(this.pseudoJson)[0]!;
            if (tag === "Ok_") {
                return new TaggedValue_("Ok_", new ping2.Output.Ok_(this.pseudoJson["Ok_"]));
            }
            return new UntypedTaggedValue_(tag, this.pseudoJson[tag]);
        }
    }
    ...
    export namespace Output {
        ...
        export class Ok_ {
            ...
            static fromTyped({        
                ...
                return new ping2.Output.Ok_(input);
            }
        }
    }
...
export class Select_ {
    ...
    }static for_ping2(select: ping2.Select_): Select_ {
        return new Select_(select.pseudoJson);
    }}
...
export class ClientInterface_ {
    ...
    async ping2(headers: Record<string, any>, input: ping2.Input): Promise<[Record<string, any>, ping2.Output]> {
        const message = await this.client.request(new Message(headers, input.pseudoJson));
        return [message.headers, new ping2.Output(message.body)];
    }
}
...
export class ServerHandler_ {
    ...
    async handler(message: Message): Promise<Message> {
        ...
        else if (functionName === "fn.ping2") {
            const [headers, output] = await this.ping2(message.headers, new ping2.Input(message.body));
            return new Message(headers, output.pseudoJson);
        }
